...
function genYtMp4Cmd(string $url, int $height = 0)
{
    ...
    return "yt-dlp -f \"bestvideo{$ql1}[vcodec^=avc1][ext=mp4]+bestaudio[ext=m4a]/bestvideo[ext=mp4]+bestaudio[ext=m4a]/best{$ql2}\" --merge-output-format mp4 $url";
}
...
{
    $url = escapeshellarg($url);
    return "yt-dlp -x --audio-format mp3 --audio-quality 0 --embed-thumbnail --embed-metadata $url";
}
